...
public class VirtualCar {
    ...
    public void active(){
        //todo 激活车辆
        com.db.Tools.writeGloablTxt("-- 激活车辆:" + " " + license + " " + obdCode + " " + vin + " " + " " + simNumber + " " + DateUtil.format(activeTime));
        ...
        com.db.Tools.writeGloablTxt(Stringtmp);
    }
    ...
    public void driveOneDay(){
        ...
        int startCount= com.db.Tools.getNoBetween(1, 7)-1;//车一天启动0~6次
        com.db.Tools.writeGloablTxt(DateUtil.format(currentDate, "-- 【yyyy-MM-dd】") + "-----" + obdCode + " " + " 启动次数 " + startCount);
        for (int i = 0; i < startCount; i++) {
            ...
            Stringtmp = Stringtmp.replace("{currentAvgOil}",String.valueOf(com.db.Tools.getNoBetween(800, 1800)));
            Stringtmp = Stringtmp.replace("{avgOil}",String.valueOf(com.db.Tools.getNoBetween(1000, 2500)));
            Stringtmp = Stringtmp.replace("{speedUp}",String.valueOf(com.db.Tools.getNoBetween(1, 2) - 1));
            Stringtmp = Stringtmp.replace("{maxSpeed}",String.valueOf(com.db.Tools.getNoBetween(10, 80)));

            Stringtmp = Stringtmp.replace("{lon}", com.db.Tools.cutString(String.valueOf(119.965034 + com.db.Tools.getNoBetween(1000, 8000) * 0.000001), 11));
            Stringtmp = Stringtmp.replace("{lat}", com.db.Tools.cutString(String.valueOf(31.8097941 + com.db.Tools.getNoBetween(1000, 8000) * 0.000001), 10));
            ...
            for (int j = 0; j < countPerTrip; j++) {
                ...
                s=s.replace("{vin}", vin);
                int speed= com.db.Tools.getNoBetween(1, 90);
                ...
                s = s.replace("{lon}", com.db.Tools.cutString(String.valueOf(119.965034 + com.db.Tools.getNoBetween(1000, 8000) * 0.000001), 11));
                s = s.replace("{lat}", com.db.Tools.cutString(String.valueOf(31.8097941 + com.db.Tools.getNoBetween(1000, 8000) * 0.000001), 10));
                s = s.replace("{direction}", String.valueOf(com.db.Tools.getNoBetween(1, 300)));
                s = s.replace("{locationTime}", DateUtil.format(DateUtil.gmt8toUTC(currentDate)));
                if(j%5==0){
                    startRecordTime=new Date(currentDate.getTime()+1000* com.db.Tools.getNoBetween(40, 150));
                }
                s = s.replace("{recordTime}", DateUtil.format(startRecordTime));
                startDistance=startDistance+ com.db.Tools.getNoBetween(20, 90);
            ...
            int time1= com.db.Tools.getNoBetween(alreadyDriveSeconds / 4, alreadyDriveSeconds / 3);
            int time2= com.db.Tools.getNoBetween(alreadyDriveSeconds / 3, alreadyDriveSeconds / 2);
            int time3=alreadyDriveSeconds-time1-time2;
            int time4=0;
            int distance1= com.db.Tools.getNoBetween(startDistance / 4, startDistance / 3);
            int distance2= com.db.Tools.getNoBetween(startDistance / 3, startDistance / 2);
